...
export async function GET() {
  ...
  if (blogsSnap.exists()) {
    ...
    urls += Object.keys(blogs)
      .map(id => {
        const lastmod = blogs[id].date ? new Date(blogs[id].date).toISOString() : new Date().toISOString();
  ...
  if (newsSnap.exists()) {
    ...
    urls += Object.keys(news)
      .map(id => {
        const lastmod = news[id].date ? new Date(news[id].date).toISOString() : new Date().toISOString();
